...
{
    public class ReviewAndRating_DAL
    {

        private readonly string _healthCareConnection;
        private readonly string foldername;
        private readonly string filename = "Reviewfile";
        public ReviewAndRating_DAL(IConfiguration configuration)
        {
        ...
        public async Task<object> getReviewRatingsData(int ProductId)
        {
            ...
            try
            {
                ...
                {
                    List<GetProductReviewAndRatingByProductIdDTO> reviewsAndRatings = new List<GetProductReviewAndRatingByProductIdDTO>();
                    ...
                    using (SqlCommand cmdForReviews = new SqlCommand("GetReviewRatings", con))
                    {
                        ...
                        using (SqlDataAdapter adapter = new SqlDataAdapter(cmdForReviews))
                        {
                            ...
                            {
                                GetProductReviewAndRatingByProductIdDTO reviews = new GetProductReviewAndRatingByProductIdDTO
                                {
                                    ReviewId = CommonServices.EncryptPassword(row["ReviewId"].ToString()),
                            ...
                            return (new { message = "Sellers ", reviewsAndRatings, ratingsArray, totalCount });
                        }
                    }
                }
            }
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, con))
                    {
                        ...
                        if (a > 0)
                        {
                            ...
                            if (updateResult <= 0)
                            {
                                ...
                                return (new { message = "product review status isn't change or not found." });
                            }
                        }
                        else
                        {
                            return (new { message = "product review data isn't Inserted Successfully." });
                        }

                        return (new { message = "Review added successfully." });
                    }
                }
            }
            catch (Exception ex)
            {
                return (new { message = "Server Error. Try Again.!!!" });
            }
        ...
        //public async Task<object> UpdateReviewAndRatings( ReviewsAndRatings reviewAndRating)
        //{
        //    try
        //    {
        //        using (SqlConnection con = new SqlConnection(_healthCareConnection))
        ...
        //            cmd.ExecuteNonQuery();
        //            con.Close();
        //            return (new { message = "Review updated successfully" });
        //        }

        //    }
        //    catch(Exception ex)
        //    {
        //        return (new { message = "Review updated Unsuccessfully." });
        //    }
